  ...
  const applyFilters = useCallback(() => {
    ...
    if (filters.budget !== "all") {
      ...
      filtered = filtered.filter((pkg) =>
        max ? pkg.price >= min && pkg.price <= max : pkg.price >= min
      );
    }

    if (filters.transport !== "all") {
      filtered = filtered.filter(
        (pkg) => pkg.transport_type === filters.transport
      );
    }

    if (filters.accommodation !== "all") {
      filtered = filtered.filter(
        (pkg) => pkg.accommodation_level === filters.accommodation
      );
    }
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center text-center py-24 bg-gray-800/20 border-2 border-dashed border-gray-700 rounded-lg">
                ...
                <Button
                  ...
                  variant="outline"
                  className="bg-transparent border-sky-500 text-sky-400 hover:bg-sky-500/10 hover:text-sky-300"
                >
                  Clear all filters
                </Button>
              </div>
